...
class Boids {
  static render(int width, int height) {
    ...
    var pointer =
        sim_param_buffer.getMappedRange(size: sim_param_data.lengthInBytes);
    Float32List _list =
        (pointer.cast<ffi.Float>()).asTypedList(sim_param_data.length);
    _list.setAll(0, sim_param_data);
    sim_param_buffer.unmap();

    var compute_bind_group_layout =
    ...
    var compute_pipeline = device.createComputePipeline(
        GPUComputePipelineDescriptor(
            ...
            compute: GPUProgrammableStage(
                module: compute_shader, entryPoint: 'main')));

    var vertex_buffer_data =
        Float32List.fromList([-0.01, -0.02, 0.01, -0.02, 0.00, 0.02]);
    ...
    var pointer1 =
        vertices_buffer.getMappedRange(size: vertex_buffer_data.lengthInBytes);
    Float32List _list1 =
        (pointer1.cast<ffi.Float>()).asTypedList(vertex_buffer_data.length);
    ...
    var _pointer0 = particle_buffers0.getMappedRange(
        size: initial_particle_data.lengthInBytes);
    Float32List _plist0 =
        (_pointer0.cast<ffi.Float>()).asTypedList(initial_particle_data.length);
    _plist0.setAll(0, initial_particle_data);
    particle_buffers0.unmap();

    var _pointer1 = particle_buffers1.getMappedRange(
        size: initial_particle_data.lengthInBytes);
    Float32List _plist1 =
        (_pointer1.cast<ffi.Float>()).asTypedList(initial_particle_data.length);
