...
class GPUQueue {
  ...
  void writeBuffer(
    ...
    if(data is Uint32List) {
      ...
      (ptr as Pointer<Uint32>).asTypedList(data.length).setAll(0, List<int>.from(data.map((e) => e.toInt())));
    } else if (data is Float32List) {
      ptr = ffi.calloc<Float>(data.length);
      (ptr as Pointer<Float>).asTypedList(data.length).setAll(0, List<double>.from(data.map((e) => e.toDouble())));
